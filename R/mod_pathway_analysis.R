    ...
    shiny::observeEvent(input$run_pathway, {
      ...
      shiny::withProgress(message = "Pathway analysis", value = 0, {
        ...
        if (is.null(res) || !nrow(res)) {
          shiny::showNotification("No DE results available. Run DE first or relax cutoffs.", type = "warning")
          return(NULL)
        }
        ...
        # ID prep: use rownames, strip Ensembl version suffix if present
        d1 <- res[, c("log2FoldChange", "padj"), drop = FALSE]
        d1$gene <- rownames(res)
        if (is.null(d1$gene)) {
          shiny::showNotification("DE results lack gene rownames. Cannot proceed.", type = "error")
          return(NULL)
        }
        d1$gene_base <- sub("\\.\\d+$", "", d1$gene)
        
        # map SYMBOL or ENSEMBL -> ENTREZ
        from_type <- if (.is_symbol(d1$gene_base)) "SYMBOL" else "ENSEMBL"
        incProgress(0.15, detail = paste0("Mapping IDs (", from_type, " -> ENTREZ)"))
        d1_ids <- suppressMessages({
          clusterProfiler::bitr(d1$gene_base, fromType = from_type, toType = "ENTREZID", OrgDb = orgdb)
        })
        ...
        d1_merged <- merge(d1, d1_ids, by.x = "gene_base", by.y = 1)
        ...
        shiny::showNotification(paste0("Genes after direction/cutoffs: ", n_keep), type = "message", duration = 4)
        
        if (!n_keep) {
          shiny::showNotification("No genes pass the selected cutoffs. Relax thresholds.", type = "warning")
          return(NULL)
        }
        ...
        if (length(geneList) < 10) {
          shiny::showNotification("Too few mapped genes (<10) for enrichment. Relax thresholds.", type = "warning")
          return(NULL)
        }
        ...
        # Enrichment
        incProgress(0.65, detail = paste0("Enrichment: ", input$pathway_db))
        pathway_result <- NULL
        ...
        if (identical(input$pathway_db, "GO")) {
          pathway_result <- tryCatch({
            clusterProfiler::enrichGO(
              ...
              pvalueCutoff = input$padj_threshold, qvalueCutoff = qcut,
              readable = TRUE
            )
          }, error = function(e) { shiny::showNotification(paste("GO enrichment failed:", e$message), type = "error"); NULL })
          ...
          }, error = function(e) { shiny::showNotification(paste("KEGG enrichment failed:", e$message), type = "error"); NULL })
          if (!is.null(kegg_raw) && nrow(as.data.frame(kegg_raw)) > 0) {
            pathway_result <- tryCatch({
              clusterProfiler::setReadable(kegg_raw, OrgDb = orgdb, keyType = "ENTREZID")
            }, error = function(e) { shiny::showNotification(paste("setReadable failed:", e$message), type = "error"); kegg_raw })
          }
          ...
          }, error = function(e) { shiny::showNotification(paste("Reactome enrichment failed:", e$message), type = "error"); NULL })
        ...
        incProgress(0.85, detail = "Post-processing terms")
    ...
    output$download_emap_plot <- shiny::downloadHandler(
      ...
      content  = function(file) {
        ...
        if (is.null(df) || nrow(df) < 2 || is.null(pr@termsim) || all(pr@termsim == 0)) {
          shiny::showNotification("No valid term similarity for export.", type = "warning")
          return()
        }
